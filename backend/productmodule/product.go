...
func (p *Module) GetProductBatch(limit int, offset int, searchNameKeyWord, searchDescriptionKeyword string) ([]ProductResponse, error) {
	var resp []ProductResponse
	var err error

	resp, err = p.Storage.GetProductBatch(limit, offset, searchNameKeyWord, searchDescriptionKeyword)
	if err != nil {
		log.Println("[ProductModule][GetProductBatch] problem getting storage data, err: ", err.Error())
		return resp, err
	}
	for i, v := range resp {
		resp[i].PriceFormat = formatPrice(v.Price)
	}
