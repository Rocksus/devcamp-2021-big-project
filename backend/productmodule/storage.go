...
func (s *storage) GetProductBatch(name string, limit int, offset int) ([]ProductResponse, error) {
	resp := make([]ProductResponse, 0)

	rows, err := s.ProductDB.Query(getProductBatchByName, name, name, limit, offset)
	if err != nil {
