...
func (s *storage) GetProductBatch(limit int, offset int, name string, desc string) ([]ProductResponse, error) {
	resp := make([]ProductResponse, 0)

	rows, err := s.ProductDB.Query(getProductBatchQuery, limit, offset, name, desc)
	if err != nil {
