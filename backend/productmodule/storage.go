...
func (s *storage) GetProductBatch(name_query string, limit int, offset int) ([]ProductResponse, error) {
	resp := make([]ProductResponse, 0)

	like_string := fmt.Sprintf("%s%v%s", "%", name_query, "%")

	rows, err := s.ProductDB.Query(getProductBatchQuery, like_string, limit, offset)
	if err != nil {
